...
with st.form("dispute_form"):
    ...
    selected_reasons = st.multiselect("Select Dispute Reason(s)", list(dispute_reasons.keys()), default=["Account not mine (identity theft)"])

    submitted = st.form_submit_button("Generate Dispute Letter")

if submitted:
    subject_line = "Re: " + " & ".join([dispute_reasons[r][0] for r in selected_reasons])
    combined_body = ""
    for reason in selected_reasons:
        combined_body += f"{dispute_reasons[reason][1]}\n\n"
